...
struct gg_event *gg_watch_fd(struct gg_session *sess)
{
	...
	switch (sess->state) {
		case GG_STATE_RESOLVING:
		{
			...
			if (send(sess->fd, &addr, sizeof(addr),0) < (signed)sizeof(addr) || addr.s_addr == INADDR_NONE) {
		...
		case GG_STATE_CONNECTING_HUB:
		{
			 ...
			 * sta³o siê co¶ z³ego. */
			if (send(sess->fd, buf, strlen(buf), 0) < (signed)strlen(buf)) {
		...
		case GG_STATE_CONNECTING_GG:
		{
			...
			if (sess->proxy_addr && sess->proxy_port) {
				...
				if (auth) {
					...
					if (send(sess->fd, auth, strlen(auth), 0) < (signed)strlen(auth)) {
						gg_debug(GG_DEBUG_MISC, "// gg_watch_fd() can't send proxy request\n");
						free(auth);
						goto fail_connecting;
					}
